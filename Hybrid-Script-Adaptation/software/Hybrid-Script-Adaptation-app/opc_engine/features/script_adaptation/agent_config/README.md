...
本目录是 `script_adaptation_agent.py` 的自包含能力目录。混剪脚本适配智能体默认只从这里读取自己的提示词、模型设置和本地密钥，不依赖 9994 的配置、运行状态或输出目录。
...
- 输入只扫描 `04爆款视频脚本/混剪-钩子` 和 `04爆款视频脚本/混剪-CTA` 两个目录中的 Markdown 脚本。Web 界面支持点击选择，CLI 使用 `--script-file /path/to/script.md`。
