(Veo工业流水线_动态分镜与实操生成引擎
  ...
  :输入 (
    ...
    (脚本处理逻辑 (
      ...
      "6. 第五阶段【补技术占位】：最后一个 Veo 片段的原始内容不足8秒时，保持原内容时长和节奏不变，在尾部补充黑屏静音技术占位。不得通过慢动作、延长停留、重复动作、补充台词、增加空镜或新增剧情凑满8秒。技术占位不属于内容分镜，不新增 shot_number，也不新增 Veo 片段。"
    ))
...
④ 纯净画面: plain frame edge to edge, empty screen area only, real-life scene fills the entire image, no overlay elements, just what the camera sensor captured.
    ...
    (C25 "固定时长技术占位铁律（最高优先级）：系统注入“固定时长技术占位硬性要求”时，必须逐字保留六个时长字段和 `[TECHNICAL_PADDING: BLACK_SILENT]`。最后一个 Veo 片段的 Motion 只执行到“有效内容结束”，随后进入纯黑画面、完全静音、无人物、无产品、无字幕、无贴纸、无动作、无转场内容的技术占位。占位不新增 JSON shot，不新增主视觉分镜，不新增片段；系统未注入占位要求时禁止主动添加黑屏。")
  )
  ;;──────────────────────────────────────────────────────────────────────
  ;; 执行指令
  ;;──────────────────────────────────────────────────────────────────────
