...
def require_markdown_path(path: Path) -> None:
    if path.suffix.lower() != ".md":
        raise SystemExit(f"混剪参考脚本输入必须是 .md 文件: {path}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--script-file", help="本次直接使用的混剪参考脚本路径，仅用于 adapt 阶段")
    parser.add_argument("--script-stdin", action="store_true", help="从 stdin 读取混剪参考脚本文本并保存到当前产品项目后适配")
