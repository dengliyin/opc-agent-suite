...
def update_config(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    hybrid = config.setdefault("hybrid", {})
    ...
    if "material_type" in payload:
        ...
        if material_type not in {"混剪-钩子", "混剪-CTA"}:
            raise ValueError("素材类型必须是“混剪-钩子”或“混剪-CTA”")
        hybrid["material_type"] = material_type
...
class WebJob:
    ...
    def _run_direct_url_download(self, payload: dict[str, Any]) -> tuple[Path, RunReport]:
        config = read_config_file(CONFIG_PATH)
        paths = ProjectPaths(ROOT, config)
        product_name = str((config.get("product") or {}).get("name") or "").strip()
        if not product_name:
            raise ValueError("请先选择或输入产品名称")
        paths.ensure()
    ...
    def do_POST(self) -> None:  # noqa: N802 - BaseHTTPRequestHandler API.
        ...
        try:
            ...
            if parsed.path == "/api/settings":
                ...
                mode = str(payload.get("mode") or "url_download")
                if mode != "url_download":
                    raise ValueError("混剪参考视频采集只支持 URL 下载")
