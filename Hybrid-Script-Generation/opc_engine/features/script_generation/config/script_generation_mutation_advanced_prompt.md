...
**音频文案铁律**：每个镜头的 **[音频文案]** 必须输出真实目标语言台词，不能只写“目标语言口播（大意：……）”“马来语口播（大意：……）”这类说明。中文翻译对照只能放在该条音频文案的最后一个括号里，禁止插在目标语言台词中间；格式必须是“完整目标语言口播 +（中文翻译对照：完整中文翻译）”，且中文翻译必须覆盖前面整段目标语言口播。正确格式示例：`*   **[音频文案]**：Ini bukan syampu biasa. Syampu ini memang khas untuk menutup uban.（中文翻译对照：这不是普通的洗发水。这款洗发水是专门用来遮盖白发的。）`

**音频长度铁律**：每个镜头的 **[音频文案]** 必须匹配该镜头时间码，不能比母版同镜头更长。原则上目标语言台词长度不得超过母版同镜头台词的 110%；1-2 秒镜头只能用一个短句或半句，3-4 秒镜头最多 1 个完整短句，超过 4 秒才允许 2 个短句。不得为了加入 persona/time_marker/卖点而扩写台词，放不下的信息必须舍弃。
...
*   **[主体]** （本变体的人物全貌：年龄、性别、肤色、脸型、体型、发型、服装、配饰。从种子值填入，全镜保持一致。）
...
*   **[音频文案]** （目标语言台词，标注语种。time_marker 融入开场句，KOC 身份代入句替换对应位置的母版台词，其余台词保持母版不变。）
