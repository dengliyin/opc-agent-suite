...
SCRIPT_OUTPUT_SOURCE_ROOT = Path(
    os.environ.get(
        ...
        VAULT_ROOT / "wiki" / "视频" / "AI实拍混剪" / "03复刻裂变脚本",
    )
).expanduser()
...
def build_generation_prompt(config):
    ...
    if material_type not in {"混剪-钩子", "混剪-CTA"}:
        material_type = next(
            (part for part in reference_path.parts if part in {"混剪-钩子", "混剪-CTA"}),
            "",
        )
    if not material_type:
        raise SystemExit("无法识别解析脚本类型；文件必须位于混剪-钩子或混剪-CTA目录")
    material_rule = (
        "只复刻开头钩子片段；不得补写中段产品介绍或结尾 CTA。"
        if material_type == "混剪-钩子"
        else "只复刻结尾 CTA 片段；不得补写开头钩子或中段产品介绍。"
    )
...
def build_mutation_prompt(config, generated_script, variant_count, batch_start=1, total_variant_count=None, reference_context=""):
    ...
    if not material_type:
        raise SystemExit("无法识别解析脚本类型；文件必须位于混剪-钩子或混剪-CTA目录")
    material_rule = (
        "每个变体都只能是开头钩子片段，不得补写 CTA 或中段产品介绍。"
        if material_type == "混剪-钩子"
        else "每个变体都只能是结尾 CTA 片段，不得补写钩子或中段产品介绍。"
    )
    country = str(config.get("script_country", "") or "").strip()
    target_language = normalized_target_language(config)
