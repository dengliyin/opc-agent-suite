...
HOT_SCRIPT_SOURCE_ROOT = Path(
    os.environ.get("HYBRID_SCRIPT_GENERATION_INPUT_ROOT", HYBRID_WORKFLOW_ROOT / "02解析脚本")
).expanduser()
SCRIPT_OUTPUT_SOURCE_ROOT = Path(
    os.environ.get("HYBRID_SCRIPT_GENERATION_OUTPUT_ROOT", HYBRID_WORKFLOW_ROOT / "03复刻裂变脚本")
).expanduser()
...
def hybrid_reference_classification(path: str | Path) -> dict[str, str]:
    reference_path = resolve_root_path(str(path))
    try:
        relative = reference_path.relative_to(HOT_SCRIPT_SOURCE_ROOT.resolve())
    except ValueError as exc:
        raise ValueError("解析脚本必须位于 02解析脚本/<类型>/<产品名>/") from exc
    if len(relative.parts) < 3:
        raise ValueError("解析脚本目录必须是 02解析脚本/<类型>/<产品名>/<脚本.md>")
    material_type, product_name = relative.parts[:2]
    if material_type not in HYBRID_MATERIAL_TYPES:
        raise ValueError("解析脚本类型必须是“混剪-钩子”或“混剪-CTA”")
    if not product_name.strip():
        raise ValueError("解析脚本缺少产品名称目录")
    return {
        "material_type": material_type,
        "product": product_name,
        "group": f"{material_type} / {product_name}",
        "source": reference_path.stem,
    }


def output_dir_for_reference(reference_path: str | Path) -> Path:
    classification = hybrid_reference_classification(reference_path)
    return (
        SCRIPT_OUTPUT_SOURCE_ROOT
...
def reference_output_stems(reference_path: str | Path) -> tuple[str, ...]:
    output_root = output_dir_for_reference(reference_path)
...
def library_payload(config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    output_stems_by_reference: dict[str, tuple[str, ...]] = {}
    ...
    for path in iter_markdown_files(HOT_SCRIPT_SOURCE_ROOT):
        try:
            classification = hybrid_reference_classification(path)
        except ValueError:
            continue
        reference_key = path.resolve().as_posix()
        if reference_key not in output_stems_by_reference:
            output_stems_by_reference[reference_key] = reference_output_stems(path)
        references.append(
            {
                "name": path.name,
                "path": path.as_posix(),
    ...
    references.sort(
        key=lambda item: (
            HYBRID_MATERIAL_TYPES.index(item["material_type"]),
            item["product"].lower(),
            item["name"].lower(),
        )
    )
...
def state_payload(refresh_library: bool = False) -> dict[str, Any]:
    ...
    if reference_path and not safe_inputs.get("output_dir"):
        safe_inputs["output_dir"] = output_dir_for_reference(reference_path).as_posix()
...
def save_state(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if reference_mode in {"script", "analysis"}:
    ...
    selected_reference = str(
        inputs.get("script_reference_analysis_path")
        or inputs.get("script_reference_script_path")
        or payload.get("reference_path")
        or ""
    ).strip()
    if selected_product_doc and selected_reference:
        product_name = product_name_from_path(selected_product_doc)
        classification = hybrid_reference_classification(selected_reference)
        if classification["product"] != product_name:
            raise ValueError(
                f"产品信息“{product_name}”与解析脚本产品目录“{classification['product']}”不一致"
            )
        inputs["output_dir"] = output_dir_for_reference(selected_reference).as_posix()
        inputs["script_hybrid_material_type"] = classification["material_type"]
...
def import_markdown_file(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if kind == "product_doc":
        ...
        state = save_state(
            {
                "reference_mode": "analysis",
                "reference_path": display,
                "script_reference_kind": "竞品视频拆解结果",
                "script_reference_script_path": "",
                "script_reference_analysis_path": display,
            }
        )
...
def script_output_roots(config: dict[str, Any]) -> list[Path]:
    roots: list[Path] = []
    try:
        add_unique_path(roots, output_dir_for_reference(get_reference_path(config)))
    except (SystemExit, ValueError):
        pass
    configured_output = str(config.get("output_dir") or "").strip()
    if configured_output:
        configured_path = resolve_root_path(configured_output)
        try:
            configured_path.relative_to(SCRIPT_OUTPUT_SOURCE_ROOT.resolve())
            add_unique_path(roots, configured_path)
        except ValueError:
            pass
    add_unique_path(roots, SCRIPT_OUTPUT_SOURCE_ROOT)
    return roots
...
def _remove_deleted_outputs_from_indexes(deleted_paths: list[Path]) -> None:
    ...
    outputs_snapshot = load_snapshot("hybrid-script-generation", "outputs")
    if outputs_snapshot:
        ...
        save_snapshot("hybrid-script-generation", "outputs", payload)
...
def clear_mutation_outputs_for_references(reference_paths: list[str], *, running: bool) -> dict[str, Any]:
    ...
    if not requested:
        raise ValueError("请先勾选要清除裂变脚本的解析脚本")
    ...
    for value in dict.fromkeys(requested):
        ...
        try:
            reference.relative_to(reference_root)
        except ValueError as exc:
            raise ValueError("只允许选择当前 02解析脚本目录内的脚本") from exc
        if reference.suffix.lower() != ".md" or not reference.is_file():
            raise ValueError("所选解析脚本不存在或不是 Markdown 文件")
        hybrid_reference_classification(reference)
        _country, author, source_id = reference_country_author_and_video_id(reference)
        identity = f"-{author}-{source_id}"
        selected_references.append(str(reference))
        output_root = output_dir_for_reference(reference)
        if not output_root.is_dir():
            continue
        matched = [path.resolve() for path in output_root.glob("裂变-*.md") if identity in path.stem]
        if matched:
            matched_references.add(str(reference))
...
class GenerationJob:
    ...
    def start(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if not reference_value:
            raise ValueError("请先选择 02解析脚本 下的钩子或 CTA Markdown")
        reference_path = resolve_root_path(reference_value)
        if reference_path.suffix.lower() != ".md" or not reference_path.is_file():
            raise ValueError("解析脚本无效或不存在，请重新选择 Markdown 文件")
        classification = hybrid_reference_classification(reference_path)
        product_doc = str(payload.get("script_product_document_path") or "").strip()
        if product_doc and product_name_from_path(product_doc) != classification["product"]:
            raise ValueError("所选产品信息与解析脚本的产品目录不一致")
        save_state(payload)
        with self.lock:
...
<body>
  <header>
    <div class="window-title">
      <div>
        <h1>钩子与 CTA 脚本复刻裂变智能体</h1>
        <div class="sub">产品资料 · 解析脚本 · 复刻 / 裂变 · 独立归档</div>
      </div>
    </div>
    <div class="bar">
      ...
      <section class="panel task-panel">
        ...
        <div id="referenceDrop" class="selected-file">
          <div class="selected-file-text">
            <strong id="referenceDropTitle">未选择解析脚本</strong>
    ...
    <section class="stack">
      <section class="panel">
        ...
        <div class="bar" style="margin-top:10px;">
          ...
          <button class="primary" id="runBtn">复刻 / 裂变脚本</button>
  ...
  <script>
    ...
    function referenceItem(path) {
      return (library.references || []).find(item => item.path === path);
    }

    function outputDirForReference(path) {
      const item = referenceItem(path);
      return item && item.output_dir ? item.output_dir : '';
    }
    ...
    function expectedClonePath() {
      ...
      const root = $('outputDir').value.trim() || outputDirForReference(referencePath);
    ...
    function updateMutationSourceHint() {
      ...
      if (!path) {
        hint.innerHTML = '<strong>裂变母稿：</strong>请先选择产品和解析脚本。';
        return;
      }
    ...
    function selectReference(path) {
      ...
      $('outputDir').value = outputDirForReference(path);
      updateDropState('reference', $('referencePath').value, 0);
      renderReferenceList();
      updateMutationSourceHint();
    }

    function updateMutationCleanupControls(refs = currentProductReferences()) {
      const paths = refs.map(item => item.path).filter(Boolean);
    ...
    function toggleAllMutationReferences() {
    ...
    function renderReferenceList() {
      ...
      list.innerHTML = '';
      if (!refs.length) {
        list.innerHTML = '<div class="meta">当前产品名下没有匹配到解析脚本。</div>';
        $('selectedScriptMeta').textContent = '请确认产品名和 02解析脚本/<类型>/<产品名> 的目录名称一致。';
        return;
      }
      ...
      for (const item of refs) {
        if (item.material_type !== currentMaterialType) {
          currentMaterialType = item.material_type || '未分类';
          const group = document.createElement('div');
          group.className = 'meta';
          group.textContent = `${currentMaterialType} / ${item.product || ''}`;
        ...
        checkbox.setAttribute('aria-label', `选择清除 ${item.name || '解析脚本'} 的裂变脚本`);
    ...
    async function clearSelectedMutations() {
      ...
      const remaining = referencePaths.length > 8 ? `\n• 另有 ${referencePaths.length - 8} 个解析脚本` : '';
      if (!confirm(`确定清除以下 ${referencePaths.length} 个解析脚本产生的全部裂变脚本吗？\n\n${names}${remaining}\n\n只删除对应裂变 Markdown；永久删除，无法恢复。“已裂变”、累计裂变次数和历史编号仍会保留。`)) return;
    ...
    function updateDropState(kind, path, chars) {
      ...
      title.textContent = hasPath ? fileLabel(path) : (isProduct ? '选择或拖入产品信息 .md' : '未选择解析脚本');
    ...
    function payload() {
      ...
      return {
        ...
        script_reference_kind: '竞品视频拆解结果',
        script_reference_script_path: '',
        script_reference_analysis_path: referencePath,
        ...
        output_dir: $('outputDir').value.trim() || outputDirForReference(referencePath),
    ...
    function renderState(data) {
      ...
      $('outputDir').value = inputs.output_dir || outputDirForReference($('referencePath').value);
    ...
    async function startRun(dryRun) {
      ...
      if (!referencePath) {
        $('logs').textContent = '请先从解析脚本列表选择一个参考脚本，再复刻或裂变。';
        return;
      }
      body.dry_run = dryRun;
    ...
    $('selectAllMutationsBtn').onclick = toggleAllMutationReferences;
...
class ScriptGenerationWebHandler(BaseHTTPRequestHandler):
    server_version = "HybridScriptGenerationAgentWeb/1.0"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the hybrid hook and CTA script clone/mutation web UI.")
...
def main() -> None:
    ...
    print(f"钩子与 CTA 脚本复刻裂变智能体 Web UI: http://{args.host}:{args.port}", flush=True)
    try:
