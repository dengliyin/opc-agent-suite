...
右侧运行面板常驻显示文字贴纸设置，默认关闭；成品文件列表不在页面展示。每个成品最多使用一个手动输入的贴纸，一次批量拼接对所选视频使用同一组配置。初始提供“黑底白字”、“黄底黑字”和“白字描边”三种风格，可选上方、中部或下方安全位置，以及全片或自定义显示时间。贴纸最多 36 个字符、最多两行，会自动调整字号并使用轻量进出场动画。字幕仍保持关闭。
