...
右侧运行面板常驻显示文字贴纸设置，默认关闭；成品文件列表不在页面展示。选中同一产品的待拼接项目后，可以按“产品 + 销售国家”读取 `wiki/视频/视频文字标签贴纸库`中的本地 Markdown 预设，选中后回填贴纸文字并显示中文释义；也可继续手动输入。“随机分配”会在当前产品和国家的文案池内，为批量中每个成品单独分配文案；预设数量足够时不重复，用完一轮后才重新随机。字体、位置和时间在同一批次中保持一致。初始提供“粗体衬线”、“气泡卡通”、“TikTok Sans”和“电影字幕”四种风格，可选上方、中部或下方安全位置，以及全片或自定义显示时间。贴纸最多 36 个字符、最多两行，会自动调整字号并使用轻量进出场动画。
