...
<body><main><header><div><h1>OPC 内容量化增长引擎</h1><p>按手动生产线路与自动发布流水线组织现有 Agent。手动 Agent 保持独立，自动流水线复用底层能力。</p></div><div class="headerTools"><button class="updateButton" onclick="openUpdate()">更新</button><a class="button" href="/settings/ai">全局 API / 模型</a><a class="button" href="/settings/paths">全局路径设置</a><div class="summary" id="summary">正在检测服务…</div></div></header><section class="workflows" id="workflows"></section><section class="destination"><div class="destinationHead"><div class="destinationTitle">统一归口 · 成品管理与发布</div><div class="destinationDescription">三条线路的最终成片统一进入成品目录，由同一个 Agent 扫描、管理和发布。</div></div><div class="flow" id="destination"></div></section><p class="note">控制台端口 8888 · 已接入 15 个 Agent</p></main><div class="overlay" id="updateOverlay"><section class="updatePanel"><div class="updateHead"><div><h2>程序更新</h2><div class="updateMessage" id="updateMessage">将从 GitHub 获取 main 最新代码，重建 Docker，并等待全部 Agent 恢复正常。</div></div><button onclick="closeUpdate()">关闭</button></div><div class="updateState" id="updateState">准备检查更新</div><ul class="dirtyList" id="dirtyList"></ul><pre class="updateLog" id="updateLog"></pre><div class="updateActions"><button id="startUpdateButton" class="primary" onclick="startUpdate()">开始更新</button></div></section></div>
<script>
...
async function startUpdate(){if(!confirm('确定从 GitHub 更新 main，并重建全部 Agent 吗？'))return;startUpdateButton.disabled=true;renderUpdate({state:'running',message:'正在启动更新服务…'});try{const r=await fetch('/api/system-update',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});const data=await r.json();renderUpdate(data)}catch(error){renderUpdate({state:'running',message:'连接暂时中断，正在等待 8888 恢复…'})}}
