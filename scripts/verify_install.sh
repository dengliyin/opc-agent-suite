...
for agent in "${AGENTS[@]}"; do
  python_bin="$ROOT_DIR/$agent/.venv/bin/python"
  if [ ! -x "$python_bin" ]; then
    echo "Missing virtual environment: $agent/.venv" >&2
    exit 1
  fi
  version="$($python_bin -c 'import sys; print(f"{sys.version_info.major}.{sys.version_info.minor}")')"
  if [ "$version" != "3.12" ]; then
    echo "$agent uses Python $version, expected 3.12" >&2
    exit 1
  fi
...
"$ROOT_DIR/Script-Generation/.venv/bin/python" -m compileall -q "$ROOT_DIR/Script-Generation/opc_engine" "$ROOT_DIR/Script-Generation/kesai_app.py"
