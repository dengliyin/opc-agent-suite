...
如果未启用裂变，同一产品、参考脚本和国家版本已有复刻稿时默认直接复用，不再重复调用模型。裂变结果不得覆盖旧文件；基础文件名已存在时，自动追加 `_002`、`_003` 等序号，原始响应 JSON 使用同一个序号。裂变每批最多 3 条，批量失败时自动减半，并只补缺失或校验失败的编号。
...
如果启用裂变，最终脚本必须保留初稿的镜头功能、情绪张力、视觉奇观底层逻辑和叙事结构；必须替换人物具体外观、服装造型、场景子类型与陈设，并在局部动作、拍摄角度/景别、光线/时间点、环境道具或环境音中至少再改变两项。

每个镜头的 `[做什么动作]` 只能用“主体”承接 `[主体]` 已定义的人物或人物群体，只写站位、动作、表情和必要的产品使用，不重复人数、外貌或服装；人物手持带货商品时使用 `[手持产品]`，不得写“图1中的该产品”等参考图称呼。
