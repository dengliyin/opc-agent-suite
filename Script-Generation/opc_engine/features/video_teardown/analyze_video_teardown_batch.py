...
def main():
    ...
    unified_mode = bool(os.environ.get("OPC_APP_CONFIG_PATH"))
    if not unified_mode:
    ...
    unified_output_dir = Path(unified_paths.get("analyze_output_dir", "")).expanduser() if unified_mode and unified_paths.get("analyze_output_dir") else None
    log(f"项目目录: {product_project_root(config)}")
    log(f"输出目录: {unified_output_dir or '当前产品项目 / hot_sources / 视频ID / teardown'}")
    log(f"模型: {args.model}")
    ...
    for index, video_path in enumerate(videos, start=1):
        ...
        try:
            ...
            output_dir = unified_output_dir if unified_output_dir else source_stage_dir(source_id, "teardown", config)
