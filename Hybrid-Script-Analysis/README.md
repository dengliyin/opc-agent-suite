...
```bash
python3 scripts/web_app.py --host 127.0.0.1 --port 10002
```
...
```text
输入：$OPC_VAULT_ROOT/wiki/视频/AI实拍混剪/01参考视频/<类型>/<产品名>/
输出：$OPC_VAULT_ROOT/wiki/视频/AI实拍混剪/02解析脚本/<类型>/<产品名>/
```

未提供全局环境变量时，独立运行的 10002 会回退到 `config/paths.local.json`。复制示例配置：
