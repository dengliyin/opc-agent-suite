...
Use this skill as a standalone video teardown agent. All configuration must live inside the `Hybrid-Script-Analysis` folder in the suite:

```text
opc-agent-suite/Hybrid-Script-Analysis/
```
...
```bash
.venv/bin/python scripts/web_app.py --port 10002
```
...
Suite queue runs move the final Markdown into `HYBRID_VIDEO_TEARDOWN_OUTPUT_ROOT` (or the `script_dir` fallback from `config/paths.local.json`).
