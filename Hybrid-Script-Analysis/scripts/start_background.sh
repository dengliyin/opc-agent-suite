...
listener_pid() {
  lsof -tiTCP:10002 -sTCP:LISTEN 2>/dev/null | head -n 1 || true
}
...
if pid="$(listener_pid)" && [[ -n "$pid" ]]; then
  echo "端口 10002 已被其他程序占用，PID: $pid"
...
while true; do
  ...
  '$PYTHON_BIN' '$PROJECT_DIR/scripts/web_app.py' --host 127.0.0.1 --port 10002 >> '$OUT_LOG' 2>> '$ERR_LOG'
  code=\$?
