...
def script_to_dict(settings: Settings, script: ScriptFile) -> Dict[str, object]:
    ...
    return {
        ...
        "country_code": script_country_code(script.product_name, script.md_path.name),
...
def script_country_code(product_name: str, md_name: str) -> str:
    match = re.search(rf"-{re.escape(product_name)}-([A-Z]{{2}})(?:-|$)", Path(md_name).stem)
    if match is None:
        return ""
    code = match.group(1)
    return code if code in ISO_ALPHA2_COUNTRY_CODES else ""
