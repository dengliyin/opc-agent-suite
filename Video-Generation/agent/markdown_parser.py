...
def build_direct_video_prompt(segment: Segment) -> str:
    ...
    return (
        "请根据第一张产品主参考图、第二张人物参考图和当前片段镜头脚本，直接生成一段真实商业带货短视频片段。"
        "第一张参考图是最高优先级的产品主参考图，用于强制锁定产品外观、颜色、结构、包装、Logo、标签和可见文字；"
        "第二张参考图只用于锁定人物外观、年龄、肤色、发型、脸部特征和整体气质，不得覆盖或改写第一张图中的产品。"
        "如果镜头脚本文字与第一张产品主参考图冲突，忽略冲突的产品文字描述，以第一张产品主参考图为准。"
