...
入口包含三套互相隔离的片段生产工作台，以及一套片段合成工作台：

- `Omni 片段产出`：纯 AI 线路。
- `Grok 片段产出`：纯 AI 线路。
- `混剪钩子与 CTA Omni 片段产出`：AI＋实拍混剪线路，只读取钩子与 CTA 的 Omni 适配脚本。
- `片段合成`：扫描已导出的标准片段归档，离线合成成品、校验并按确认清理中间素材。
...
标准片段产出的“删除归档”会删除 `06合成工作区` 中所选归档及 `04适配脚本` 中对应模型的适配稿，但保留 `03产品脚本` 原稿，并在 `03产品脚本/.opc/adaptation_retirements.json` 按产品、文件名和模型写入淘汰记录。脚本适配 Agent 会把该模型显示为“已淘汰”，不会再纳入“适配未适配”。

混剪 Omni 采用同一规则，但目录对应为：删除 `08混剪工作区/片段产出归档` 和混剪 `04适配脚本` 中的适配稿，保留 `03复刻裂变脚本` 原稿，并将淘汰记录写入 `03复刻裂变脚本/.opc/adaptation_retirements.json`，供 9999 使用。

片段合成已经并入本 Agent，通过 `http://127.0.0.1:9995/assembly` 进入。该工作台负责扫描、拼接、成品校验和已拼接素材清理；归档列表只读取导出记录并显示“已清理”，不再提供删除媒体的入口。
...
例如脚本目录为 `LUX-轻奢戒指/`，参考图可以命名为 `LUX-轻奢戒指-RG001-银色六爪.png`、`LUX-轻奢戒指-RG002-玫瑰金排钻.png`。页面会在产品分组内要求选择本批脚本使用的 SKU；只有一张匹配参考图时自动选择。人物图功能也会把所选产品参考图作为图1输入；需要继承旧人物时，历史人物图从图2开始。所有生成功能都会在执行前校验选择。
