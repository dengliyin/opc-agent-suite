...
优先读取 `成品视频/产品/文件.mp4`，同时兼容该根目录内旧的 `模型/日期/产品/文件.mp4`。视频类型由文件名识别，不再设置视频类型目录；新旧结构存在同一产品、同名文件时只展示新结构，新结构的日期取文件修改日期。
...
```text
$OPC_VAULT_ROOT/wiki/视频/成品视频/视频标题库
```
...
商品 ID、店铺映射和商品简称保存在本机共享配置：

```text
config/product_mappings.json
```

该文件由 9996 和 10005 共享，在第一次保存商品映射时自动创建，并由 `.gitignore` 排除，不上传 GitHub。不同电脑分别维护自己的商品映射库。比特浏览器账号、每日目标等本机发布配置保存在：
