...
def script_library_root(config: dict[str, Any] | None = None) -> Path | None:
    config = config or load_local_agent_config()
    value = str(config.get("script_adaptation_input_dir") or "").strip()
    if not value:
        return None
    path = Path(value).expanduser()
    if not path.is_absolute():
...
def script_scan_signature(path: Path, config: dict[str, Any], root: Path | None = None) -> str:
    stat = path.stat()
    source_root = root or script_library_root(config) or path.parent
    target_model = normalize_target_model(config.get("script_adaptation_target_model"))
    identity = [
        path.as_posix(),
        stat.st_mtime_ns,
...
def indexed_script_file_payload(
    ...
    payload["scan_signature"] = script_scan_signature(path, config, root)
    payload["temperature"] = "cold" if payload.get("adapted") or payload.get("retired") else "hot"
    return payload
...
def list_product_scripts(target_model: str | None = None) -> dict[str, Any]:
    ...
    config["_adaptation_retirements"] = load_adaptation_retirements(root)
    grouped_files: dict[str, list[Path]] = {}
    group_paths: dict[str, Path] = {}
    for script_path in sorted(root.rglob("*.md"), key=lambda item: item.relative_to(root).as_posix()):
        if not script_path.is_file():
            continue
        product_name = workflow.product_folder_from_script(config, script_path)
        grouped_files.setdefault(product_name, []).append(script_path)
        group_paths.setdefault(product_name, script_path.parent)

    products: list[dict[str, Any]] = []
    for product_name in sorted(grouped_files):
        products.append(product_script_payload(product_name, group_paths[product_name], grouped_files[product_name], root, config))
    ...
    payload["scan_state"] = {
        ...
        "cold": adapted_count + retired_count,
    }
    return payload
...
def list_product_scripts_incremental(target_model: str | None = None) -> dict[str, Any]:
    ...
    records, stats = incremental_records(
        ...
        source_signature=lambda path: script_scan_signature(path, config, root),
        build_record=lambda path: indexed_script_file_payload(path, root, config, status_logs),
        is_cold=lambda record: bool(record.get("adapted") or record.get("retired")),
    )
    ...
    retired_count = sum(1 for record in records if record.get("retired"))
    ...
    return {
        ...
        "scan_state": {
            ...
            "mode": "incremental",
            **stats,
            "hot": total_count - adapted_count - retired_count,
            "cold": adapted_count + retired_count,
        },
    }
...
def validate_markdown_filename(filename: str) -> str:
    ...
    if Path(name).suffix.lower() != ".md":
        raise RuntimeError("产品视频脚本输入必须是 .md 格式的文档")
    return name
...
def cached_product_scripts(target_model: str | None = None, *, refresh: bool = False, full: bool = False) -> dict[str, Any]:
    ...
    payload = cached_or_empty("script-adaptation", f"scripts-{target}", empty)
    return overlay_retired_scripts(payload, root, target)


def overlay_retired_scripts(payload: dict[str, Any], root: Path | None, target_model: str) -> dict[str, Any]:
    if not root:
        return payload
    registry = load_adaptation_retirements(root)

    def apply(script: dict[str, Any], product_name: str) -> None:
        source = root / product_name / str(script.get("name") or "")
        retired = bool(retired_model_record(root, source, target_model, registry))
...
class AgentWebJob:
        ...
        if root:
            ...
            for script in selected_scripts:
                ...
                for target_model in target_models:
                    if retired_model_record(root, Path(source_path), target_model, registry):
    ...
    def selected_scripts(self, payload: dict[str, Any]) -> list[dict[str, str]]:
        ...
        if not script_text:
            raise RuntimeError("请先选择并读取一个 .md 格式的产品视频脚本文档")
        return [
            {
                "text": script_text,
                "filename": script_filename,
...
<body>
  <header>
    <div>
      <h1>脚本适配智能体</h1>
      <div class="sub">成品脚本 · 多模型适配 · 本地归档</div>
    </div>
    <div class="statusBar">
      ...
      <div class="panelBody scriptBody">
        <div class="scriptLibrary">
          <div class="libraryHead">
            <span id="libraryTitle">产品脚本库</span>
        ...
        <div class="selectedScriptInfo">
          ...
          <span id="selectedScriptMeta">请从上方产品脚本库勾选脚本后运行适配</span>
  ...
  <script>
    ...
    function renderProducts(data) {
      ...
      $('libraryTitle').textContent = `产品脚本库 · 共 ${data.total_count || 0} · 已适配 ${data.adapted_count || 0} · 已淘汰 ${data.retired_count || 0} · 待处理 ${data.unused_count || 0} · 异常 ${data.invalid_count || 0}`;
    ...
    function updateSelectedScriptInfo() {
      ...
      if (!scripts.length) {
        ...
        $('selectedScriptMeta').textContent = '请从上方产品脚本库勾选脚本后运行适配';
...
def main() -> None:
    ...
    print(f"脚本适配智能体 Web 界面: http://{args.host}:{args.port}", flush=True)
