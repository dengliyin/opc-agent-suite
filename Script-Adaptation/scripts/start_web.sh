...
PID_FILE="${LOG_DIR}/Script-Adaptation-web-${PORT}.pid"
OUT_LOG="${LOG_DIR}/Script-Adaptation-web-${PORT}.out.log"
ERR_LOG="${LOG_DIR}/Script-Adaptation-web-${PORT}.err.log"
LABEL="com.kesai.Script-Adaptation.${PORT}"
PLIST="${HOME}/Library/LaunchAgents/${LABEL}.plist"
GUI_DOMAIN="gui/$(id -u)"
PYTHON_BIN="${PYTHON_BIN:-${SKILL_ROOT}/.venv/bin/python}"
SCREEN_NAME="Script-Adaptation-${PORT}"
...
if [ "${SCRIPT_ADAPTATION_FOREGROUND:-}" = "1" ]; then
...
if [ -z "$PID" ] || ! kill -0 "$PID" 2>/dev/null; then
  echo "脚本适配智能体启动失败，错误日志：$ERR_LOG" >&2
  tail -n 80 "$ERR_LOG" >&2 || true
  exit 1
fi

echo "脚本适配智能体 Web 界面: http://127.0.0.1:${PORT}"
echo "PID: ${PID}"
echo "日志: ${OUT_LOG}"
